...
pygame.init()
SCREEN = WIDTH, HEIGHT = 800, 600  # Set the window dimensions for your computer screen

# Initialize Pygame mixer
pygame.mixer.init()
script_dir = os.path.dirname(os.path.realpath(__file__)) # Get the path to the script
...
for i in range(1, 5):
    if i == 2:
        ...
        ext = "jpg"
    img = pygame.image.load(os.path.join(script_dir, f"Assets/Background/bg{i}.{ext}"))
...
p = Player(win)
score_card = ScoreCard(400, 40, win)
...
# Messages
title_font = os.path.join(script_dir, "Fonts/Robus-BWqOd.otf")
dodgy = Message(400, 90, 100, "Angry", title_font, WHITE, win)
walls = Message(400, 145, 80, "Walls", title_font, WHITE, win)

tap_to_play_font = os.path.join(script_dir, "Fonts/DebugFreeTrial-MVdYB.otf")
tap_to_play = Message(400, 400, 32, "TAP TO PLAY", tap_to_play_font, WHITE, win)
tap_to_replay = Message(400, 400, 30, "Tap to Replay", tap_to_play_font, WHITE, win)
...
while running:
    ...
    if home_page:
        ...
        if bird_dead and len(destruct_group) == 0:
            score_page = True
            font = os.path.join(script_dir, "Fonts/BubblegumSans-Regular.ttf")
            if score < high_score:
                score_msg = Message(400, 60, 55, "Score", font, WHITE, win)
            else:
                score_msg = Message(400, 60, 55, "New High", font, WHITE, win)

            score_point = Message(400, 110, 45, f"{score}", font, WHITE, win)
